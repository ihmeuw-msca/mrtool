...
def test_covmodel_beta_gprior(prior_beta_gaussian, use_spline, mrdata):
    covmodel = CovModel(
        "cov1", use_spline=use_spline, prior_beta_gaussian=prior_beta_gaussian
    )
    covmodel.attach_data(mrdata)
...
def test_covmodel_beta_lprior(prior_beta_laplace, use_spline, mrdata):
    covmodel = CovModel(
        "cov1", use_spline=use_spline, prior_beta_laplace=prior_beta_laplace
    )
    covmodel.attach_data(mrdata)
...
def test_covmodel_beta_uprior(prior_beta_uniform, use_spline, mrdata):
    covmodel = CovModel(
        "cov1", use_spline=use_spline, prior_beta_uniform=prior_beta_uniform
    )
    covmodel.attach_data(mrdata)
...
@pytest.mark.parametrize("use_spline", [True, False])
@pytest.mark.parametrize("use_re_mid_point", [True, False])
def test_covmodel_gamma_gprior(
    prior_gamma_gaussian, use_spline, use_re, use_re_mid_point, mrdata
):
    covmodel = CovModel(
        "cov1",
        use_spline=True,
...
@pytest.mark.parametrize("use_spline", [True, False])
@pytest.mark.parametrize("use_re_mid_point", [True, False])
def test_covmodel_gamma_lprior(
    prior_gamma_laplace, use_spline, use_re, use_re_mid_point, mrdata
):
    covmodel = CovModel(
        "cov1",
...
@pytest.mark.parametrize("use_spline", [True, False])
@pytest.mark.parametrize("use_re_mid_point", [True, False])
def test_covmodel_gamma_uprior(
    prior_gamma_uniform, use_spline, use_re, use_re_mid_point, mrdata
):
    covmodel = CovModel(
        "cov1",
        use_spline=use_spline,
...
@pytest.mark.parametrize(
    "prior_spline_monotonicity", ["increasing", "decreasing", None]
)
...
@pytest.mark.parametrize(
    "prior_spline_maxder_gaussian", [np.array([0.0, 1.0]), None]
)
@pytest.mark.parametrize(
    "prior_spline_maxder_uniform", [np.array([0.0, 1.0]), None]
)
def test_covmodel_spline_prior(
    use_spline,
    ...
    num_constraints = (
        (prior_spline_monotonicity is not None)
        + (prior_spline_convexity is not None)
    ) * prior_spline_num_constraint_points
    num_constraints += (
        prior_spline_maxder_uniform is not None
    ) * covmodel.spline.num_intervals
    num_regularizers = (
        prior_spline_maxder_gaussian is not None
    ) * covmodel.spline.num_intervals
...
def test_use_spline_intercept(mrdata):
    ...
    assert (
        linear_cov_model.num_x_vars == linear_cov_model.spline.num_spline_bases
    )
    ...
    assert c_mat.shape == (
        linear_cov_model.prior_spline_num_constraint_points,
        linear_cov_model.num_x_vars,
    )
