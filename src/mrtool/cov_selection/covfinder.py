...
class CovFinder:
        ...
        if prior_type == "Laplace":
            cov_models = [
                LinearCovModel(
                    ...
                    if cov not in self.selected_covs
                    else None,
    ...
    def summary_gaussian_model(
        ...
        beta_sig = self.is_significance(
            beta_samples, var_type="beta", alpha=self.alpha
        )
    ...
    def select_covs_by_laplace(self, laplace_std: float, verbose: bool = False):
        ...
        for i, cov in enumerate(self.all_covs):
            if (
                np.abs(laplace_model.beta_soln[i]) > self.laplace_threshold
                and cov not in self.selected_covs
            ):
        ...
        if len(additional_covs) > 0:
            ...
            beta_soln_mean, beta_soln_std, beta_soln_sig = (
                self.summary_gaussian_model(gaussian_model)
            )
            if verbose:
            ...
            for i in range(i_start, i_end):
                if beta_soln_sig[i]:
                    ...
                    self.update_beta_gprior(
                        [candidate_covs[i]],
                        np.array(
                            [0.0 if self.bias_zero else beta_soln_mean[i]]
                        ),
    ...
    def update_beta_gprior(
        ...
        for i, cov in enumerate(covs):
            if cov not in self.all_covs:
                raise ValueError(
                    f"Unrecognized covaraite {cov} for beta Gaussian prior update."
                )
            if cov not in self.beta_gprior:
