...
class MRData:
    ...
    def __post_init__(self):
        ...
        self.obs_se = expand_array(
            self.obs_se, (self.num_points,), 1.0, "obs_se"
        )
        self.study_id = expand_array(
            self.study_id, (self.num_points,), "Unknown", "study_id"
        )
        self.data_id = expand_array(
            self.data_id,
            (self.num_points,),
    ...
    def _get_cov_scales(self):
        ...
        if self.is_empty():
            self.cov_scales = {
                cov_name: np.nan for cov_name in self.covs.keys()
            }
        else:
            self.cov_scales = {
            ...
            zero_covs = [
                cov_name
                for cov_name, cov_scale in self.cov_scales.items()
                if cov_scale == 0.0
            ]
            for cov_name in zero_covs:
                warnings.warn(
                    f"numbers in covariate[{cov_name}] are all zero. "
    ...
    def is_cov_normalized(self, covs: list[str] | str | None = None) -> bool:
        ...
        for cov_name in covs:
            ok = ok and (
                (not is_numeric_array(self.covs[cov_name]))
                or (np.max(np.abs(self.covs[cov_name])) == 1.0)
            )
        return ok
    ...
    def load_df(
        ...
        self.obs = (
            empty_array() if col_obs is None else data[col_obs].to_numpy()
        )
        self.obs_se = (
            empty_array() if col_obs_se is None else data[col_obs_se].to_numpy()
        )
        self.study_id = (
            empty_array()
            if col_study_id is None
            else data[col_study_id].to_numpy()
        )
        self.data_id = (
            empty_array()
            if col_data_id is None
            else data[col_data_id].to_numpy()
        )
        self.covs = (
            dict()
            if col_covs is None
            else {cov_name: data[cov_name].to_numpy() for cov_name in col_covs}
        )
    ...
    def load_xr(
        ...
        self.obs = (
            empty_array() if var_obs is None else data[var_obs].data.flatten()
        )
        self.obs_se = (
            empty_array()
            if var_obs_se is None
            else data[var_obs_se].data.flatten()
        )
        ...
        self.covs = (
            ...
            if var_covs is None
                ...
                cov_name: data[cov_name].data.flatten() for cov_name in var_covs
            }
        )
    ...
    def to_df(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame(
            {"obs": self.obs, "obs_se": self.obs_se, "study_id": self.study_id}
        )
    ...
    def _assert_has_studies(self, studies: list[Any] | Any):
        ...
        if not self.has_studies(studies):
            studies = to_list(studies)
            missing_studies = [
                study for study in studies if study not in self.studies
            ]
            raise ValueError(
