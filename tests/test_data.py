    ...
    example_dataset = xr.Dataset(
        {
            "y": xr.DataArray(
                ...
                coords={"age_group_id": [2, 3], "location_id": [6, 102]},
            ),
            "y_se": xr.DataArray(
                np.ones([2, 2]),
                dims=["age_group_id", "location_id"],
...
def test_obs(df, obs, obs_se):
    d = MRData()
    d.load_df(
        df, col_obs=obs, col_obs_se=obs_se, col_covs=["cov0", "cov1", "cov2"]
    )
...
@pytest.mark.parametrize("covs", [None, ["cov0", "cov1", "cov2"]])
...
@pytest.mark.parametrize(
    "study_id", [None, np.array([0, 0, 1, 1, 2]), np.array([2, 0, 0, 1, 1])]
)
...
def test_get_covs(df):
    ...
    for cov_name in ["cov0", "cov1", "cov2"]:
        assert np.allclose(
            d.get_covs(cov_name), df[cov_name].to_numpy()[:, None]
        )
...
def test_load_xr(xarray):
    ...
    assert np.allclose(
        np.sort(d.obs_se), np.sort(xarray["y_se"].data.flatten())
    )
    assert np.allclose(
        np.sort(d.covs["sdi"]), np.sort(xarray["sdi"].data.flatten())
    )
    assert np.allclose(
        np.sort(d.studies), np.sort(xarray.coords["location_id"])
    )


@pytest.mark.parametrize(
    "index", [np.array([True, True, False, False, False]), np.array([0, 1])]
)
...
@pytest.mark.parametrize(
    ("studies", "result"),
    [([0, 1, 2], True), ([3, 4, 0], False), (0, True), (3, False)],
)
