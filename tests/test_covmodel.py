...
def test_covmodel_default():
    ...
    assert np.all(covmodel.prior_spline_maxder_gaussian == np.repeat(DUMMY_GPRIOR, covmodel.spline_knots.size - 1,
                                                                     axis=1))
    assert np.all(covmodel.prior_spline_maxder_uniform == np.repeat(DUMMY_UPRIOR, covmodel.spline_knots.size - 1,
                                                                    axis=1))

    # spline settings
    assert np.all(covmodel.spline_knots == np.linspace(0.0, 1.0, 4))
...
def test_covmodel_spline(spline_knots, spline_degree, spline_l_linear, spline_r_linear,
    ...
    assert all(covmodel.spline_knots == np.array([0.0, 0.25, 0.75, 1.0]))
    ...
    spline = XSpline(covmodel.spline_knots,
                     covmodel.spline_degree,
                     l_linear=covmodel.spline_l_linear,
                     r_linear=covmodel.spline_r_linear)

    assert covmodel.num_x_vars == spline.num_spline_bases - 1
...
def test_covmodel_beta_gprior(prior_beta_gaussian, use_spline):
    covmodel = CovModel('cov0', use_spline=True, prior_beta_gaussian=prior_beta_gaussian)
...
def test_covmodel_beta_lprior(prior_beta_laplace, use_spline):
    covmodel = CovModel('cov0', use_spline=True, prior_beta_laplace=prior_beta_laplace)
...
@pytest.mark.parametrize('use_spline', [True, False])
def test_covmodel_beta_uprior(prior_beta_uniform, use_spline):
    covmodel = CovModel('cov0', use_spline=True, prior_beta_uniform=prior_beta_uniform)
...
@pytest.mark.parametrize('use_spline', [True, False])
@pytest.mark.parametrize('use_re_mid_point', [True, False])
def test_covmodel_gamma_gprior(prior_gamma_gaussian, use_spline, use_re, use_re_mid_point):
    covmodel = CovModel('cov0', use_spline=True, prior_gamma_gaussian=prior_gamma_gaussian,
...
@pytest.mark.parametrize('use_spline', [True, False])
@pytest.mark.parametrize('use_re_mid_point', [True, False])
def test_covmodel_gamma_lprior(prior_gamma_laplace, use_spline, use_re, use_re_mid_point):
    covmodel = CovModel('cov0', use_spline=True, prior_gamma_laplace=prior_gamma_laplace,
                        use_re=use_re, use_re_mid_point=use_re_mid_point)
...
@pytest.mark.parametrize('use_spline', [True, False])
@pytest.mark.parametrize('use_re_mid_point', [True, False])
def test_covmodel_gamma_uprior(prior_gamma_uniform, use_spline, use_re, use_re_mid_point):
    covmodel = CovModel('cov0', use_spline=True, prior_gamma_uniform=prior_gamma_uniform,
                        use_re=use_re, use_re_mid_point=use_re_mid_point)
...
def test_covmodel_spline_prior(use_spline,
    ...
    num_constraints += (prior_spline_maxder_uniform is not None)*spline.num_intervals
    num_regularizers = (prior_spline_maxder_gaussian is not None)*spline.num_intervals
